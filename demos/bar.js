...
bar.start()
bar.message('bounce — plain')

at(3000, () => { bar.mode = 'loop';         bar.message('loop — plain') })
at(6000, () => { bar.mode = 'loop-reverse'; bar.message('loop-reverse — plain') })
at(9000, () => { bar.mode = 'bounce' })

at(9000, () => {
  bar.prefix = '|'
  bar.suffix  = '|'
  bar.message('bounce — pipe clamps')
})
at(12000, () => {
  bar.prefix = '«'
  bar.suffix  = '»'
  bar.message('bounce — guillemet clamps')
})
at(15000, () => {
  bar.prefix = '['
  bar.suffix  = ']'
...
at(15000, () => { bar.colors = C.blueRed;  bar.message('bounce — blue → red') })
at(18000, () => { bar.colors = C.rainbow;  bar.message('bounce — rainbow') })

at(21000, () => {
  bar.colorFill = true
  bar.shimmer   = 0.8
  bar.message('bounce — rainbow + shimmer + dim fill')
})
at(24000, () => {
  bar.shimmer   = 0
  bar.colorFill = false
  bar.bgColors  = ['#1a0000', '#00001a']
  bar.message('bounce — rainbow + dark bg track')
})
...
at(27000, () => {
  ...
  bar.colorFill  = true
  bar.mode       = 'loop'
  bar.message('loop — rainbow cycling + shimmer + fill')
})
...
at(30000, () => {
  ...
  bar.message('bounce — onBounce callback (updates back label)')
})
...
at(33000, () => {
  ...
  bar.empty      = ' '
  bar.progress   = 0
  bar.onComplete = () => bar.message('complete!')
  bar.message('determinate — rainbow + shimmer')
})
fill(33200, 4500, bar)
...
at(38500, () => {
  ...
  bar.colorFill = true
  bar.progress  = 0
  bar.message('determinate — heat + fill + bg')
})
fill(38700, 4500, bar)
